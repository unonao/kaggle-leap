...
class Scaler:
    ...
    def filter_and_scale(self, x, y):
        ...
        mask = (y >= self.y_lower_bound - self.eps) & (
            y <= self.y_upper_bound + self.eps
        )
...
class MaskedMSELoss(nn.Module):
    ...
    def forward(self, input, target, mask=None):
        if mask is None:
            ...
            # バッチ内の各要素について、マスクされていない要素の数を計算
            num_valid_elements = mask.sum(dim=1)

            # マスクされた要素のみでMSEを計算
            squared_error = (masked_input - masked_target) ** 2
            sum_squared_error = squared_error.sum(dim=1)

            # マスクされていない要素の数で割って、平均を計算
            mean_squared_error = sum_squared_error / num_valid_elements

            # バッチ全体の平均を計算
            loss = mean_squared_error.mean()
...
class LeapLightningModule(LightningModule):
    def __init__(self, cfg):
        ...
        self.scaler = Scaler(cfg)
        self.loss_fc = MaskedMSELoss()  # Using MSE for regression
        self.model_ema = None
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.loss_fc(
            out[:, self.use_cols_index],
            y[:, self.use_cols_index],
            mask[:, self.use_cols_index],
        )
