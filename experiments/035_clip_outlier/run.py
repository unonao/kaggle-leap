...
class Scaler:
    def __init__(self, cfg):
        ...
        self.y_nanmin = np.load(
            Path(cfg.exp.scale_dir) / f"y_nanmin_{cfg.exp.norm_name}.npy"
        )
        self.y_nanmax = np.load(
            Path(cfg.exp.scale_dir) / f"y_nanmax_{cfg.exp.norm_name}.npy"
        )
    ...
    def filter_and_scale(self, x, y):
        ...
        """
        filter_bool = np.ones(y.shape[0], dtype=bool)
