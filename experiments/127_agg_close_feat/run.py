...
def haversine_distance_sim(lat1, lon1, lat2, lon2):
    # 地球の半径（キロメートル）
    R = 1.0  # 6371.0 # どうせ打ち消し合って消えるので形だけ残す
    ...
    a = np.clip(a, 0, 1)  # 誤差で若干超える時があるので修正
    c = 2 * np.arctan2(np.sqrt(a), np.sqrt(1 - a))

    distance = R * c

    max_distance = R * np.pi
    return 1 - distance / max_distance


def retain_top_k_per_row(matrix, k=10):
    result_matrix = np.zeros_like(matrix)
    for i in range(matrix.shape[0]):
        top_k_indices = np.argsort(matrix[i])[-k:]
        result_matrix[i, top_k_indices] = matrix[i, top_k_indices]
    return result_matrix


def create_similarity_matrix(power=2.0, top_k=8, use_self=False) -> torch.Tensor:
    ...
    # 距離行列の作成
    similarity_matrix = np.zeros((384, 384))
    for i in range(384):
        for j in range(384):
            if i != j or use_self:
                similarity_matrix[i, j] = haversine_distance_sim(
                    latitude[i], longitude[i], latitude[j], longitude[j]
                )
    similarity_matrix = retain_top_k_per_row(similarity_matrix, top_k)

    # 大きさを揃える
    similarity_matrix = similarity_matrix**power
    similarity_matrix = similarity_matrix / similarity_matrix.sum(axis=0)[:, None]

    return torch.tensor(similarity_matrix, dtype=torch.float32, requires_grad=False)
...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.similarity_matrix = create_similarity_matrix(
            power=power, top_k=top_k, use_self=use_self
        )  # (384, 384)
    ...
    def forward(self, x, x_cat):
        ...
        x: (batch, 384, dim)
        """
        similarity_matrix = self.similarity_matrix.to(x.device)
        ...
        # batch を戻して処理
        x = x.view(
            -1, 384, 60, self.same_height_hidden_sizes[-1]
        )  # -> (batch, 384, 60, same_height_hidden_sizes[-1])
        x_agg = torch.einsum("ij, bjkl->bikl", similarity_matrix, x)
        x = torch.concat([x, x_agg], dim=3)
        x = x.flatten(
            start_dim=0, end_dim=1
        )  # ->(batch*384, 60, 2*same_height_hidden_sizes[-1])
        ...
        # batch を戻して処理
        x = x.view(
            -1, 384, 60, self.n_base_channels
        )  # -> (batch, 384, 60, n_base_channels)
        x_agg = torch.einsum("ij, bjkl->bikl", similarity_matrix, x)
        x = torch.concat([x, x_agg], dim=3)
        x = x.flatten(start_dim=0, end_dim=1)  # ->(batch*384, 60, 2*n_base_channels)
