...
class Scaler:
    def __init__(self, cfg, input_scale=True, output_scale=True):
        ...
        self.replace_target = cfg.exp.replace_target

        self.x_mean = np.load(Path(cfg.exp.scale_dir) / "x_nanmean.npy")
        self.x_std = np.maximum(
            np.load(Path(cfg.exp.scale_dir) / "x_nanstd.npy"), self.eps
        )
