...
class Scaler:
    ...
    def scale_input(self, x):
        ...
        if self.cfg.exp.scale.temperture:
            x[:, 0:60] = (x[:, 0:60] - 273.15) / 50
        else:
            x[:, 0:60] = (
                x[:, 0:60] - self.feat_mean_dict["base"][:60]
            ) / self.feat_std_dict["base"][:60]
        if self.cfg.exp.scale.water:
            x[:, 60:240] = np.log1p(x[:, 60:240] * 10000.0)
        else:
            x[:, 60:240] = (
                x[:, 60:240] - self.feat_mean_dict["base"][60:240]
            ) / self.feat_std_dict["base"][60:240]
        if self.cfg.exp.scale.wind:
            x[:, 240:360] = x[:, 240:360] / 50
        else:
            x[:, 240:360] = (
                x[:, 240:360] - self.feat_mean_dict["base"][240:360]
            ) / self.feat_std_dict["base"][240:360]
