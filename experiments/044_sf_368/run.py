...
class LeapModel(nn.Module):
    def __init__(
        ...
        for hidden_size in same_height_hidden_sizes + [368]:
        ...
        self.encoder = SqueezeformerEncoder(encoder_dim=368, **encoder_params)

        self.pooling = None
        if last_pooling == "avg":
            self.pooling = nn.AdaptiveMaxPool2d((1, 368))
        elif last_pooling == "max":
            self.pooling = nn.AdaptiveAvgPool2d((1, 368))
    ...
    def forward(self, x):
        ...
        # (batch, 60, 25+embedding_dim) -> (batch, 60, 368)
        x = self.same_height_encoder(x)

        x = self.encoder(x)

        # (batch, 60, 368) -> (batch, 1, 368)
        x = self.pooling(x)
