...
class LeapModel(nn.Module):
    ...
    def forward(self, x, x_cat):
        ...
        x_out, class_logits = self.unet_in(x)
        x = torch.cat([x, x_out], dim=1)

        x_top, _ = self.unet_top(x[:, :, : self.split_height, :])
        x_bottom, _ = self.unet_bottom(x[:, :, self.split_height :, :])
        x_out = torch.cat([x_top, x_bottom], dim=2)
        x = torch.cat([x, x_out], dim=1)
