...
def get_lat_lon():
    ...
    lat_lon = np.stack(
        [latitude_sin, latitude_cos, longitude_sin, longitude_cos], axis=1
    )
    return lat_lon
...
class Scaler:
    def __init__(self, cfg):
        ...
        self.lat_lon = get_lat_lon()

    def process_features(self, x_array, location_ids, time_second):
        ps = x_array[:, 360]
        ...
        # location
        feats.append(self.lat_lon)

        # time (lat_lonの0次元目分複製)
        time_array = np.array(
            [
                np.sin(2 * np.pi * time_second / (3600 * 24)),
                np.cos(2 * np.pi * time_second / (3600 * 24)),
            ]
        )
        time_array = np.tile(time_array, (384, 1))
        feats.append(time_array)
    ...
    def scale_input(self, x, location_ids, time_second):
        """
        prepare
        """
        x, x_cat = self.process_features(x, location_ids, time_second)
    ...
    def filter_and_scale(self, x, y, location_ids: np.ndarray, time_second: int):
        filter_bool = np.all(
            y <= 1e60, axis=1
        )  # y が lower_bound と upper_bound の間に収まらなければその行をスキップしていた

        x, x_cat = self.scale_input(x, location_ids, time_second)
...
class LeapLightningDataModule(LightningDataModule):
    ...
    class Val2Dataset(Dataset):
        ...
        def __getitem__(self, index):
            ...
            location_ids = np.arange(384)
            offset = ((10091520 - 641280) // 384) * (1200 * 7) % (3600 * 24)
            time_second = (offset + index * 1200) % (3600 * 24)
            x, x_cat = self.scaler.scale_input(original_x, location_ids, time_second)
            return (
                torch.from_numpy(x),
    ...
    class TestDataset(Dataset):
        ...
        def __getitem__(self, index):
            original_x = self.x[index * 384 : (index + 1) * 384]
            location_ids = np.arange(384)
            time_second = (index * 1200 * 30) % (3600 * 24)
            x, x_cat = self.scaler.scale_input(original_x, location_ids, time_second)
            return (
                torch.from_numpy(x),
                torch.from_numpy(x_cat),
    ...
    def _make_dataset(self, mode="train"):
        ...
        dataset = (
            dataset.decode()
            .to_tuple("__key__", "input.npy", "output.npy")
            .with_length(file_size)
        )

        def _process(source):
            for sample in source:
                key, original_x, original_y = sample
                location_ids = np.arange(384)
                time_second = int(key.split("-")[-1])
                original_x = np.delete(original_x, 375, 1)
                x, x_cat, y, mask = self.scaler.filter_and_scale(
                    original_x, original_y, location_ids, time_second
                )
...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.positional_embedding2 = nn.Embedding(num_embeddings, embedding_dim)
        self.constant_encoder = nn.Linear(1, 60)
        self.categorical_embedding = nn.Embedding(240, categorical_embedding_dim)

        previous_size = (
            9 + 16 + 6 + embedding_dim
        )  # 9: sequence, 16: scalar, 6: location+time
        ...
        out_base_channels = 2 * n_base_channels + embedding_dim
        self.t_head = MLP(
            9 + out_base_channels,
    ...
    def forward(self, x, x_cat):
        ...
        x_locate_time = x[:, -6:].unsqueeze(-1)
        x_locate_time = self.constant_encoder(x_locate_time).transpose(1, 2)
        input_list.append(x_locate_time)
        ...
        3. 各地域ごとに出力を作成
        """
        x_position2 = self.positional_embedding2(
            torch.LongTensor(range(60)).repeat(x.shape[0], 1).to(x.device)
        )
        x = torch.cat([x, x_position2], dim=2)
        out_t = self.t_head(
            torch.cat(
                [
                    x_state_t,
