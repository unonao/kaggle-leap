...
class CellConv(MessagePassing):
    """
    384*60頂点の各セルについて、畳み込む
    """

    def __init__(self, base_dim, edge_dim):
        super().__init__(aggr="add")
        self.base_dim = base_dim
        self.edge_dim = edge_dim
        self.edge_linear = nn.Linear(edge_dim, base_dim)
        ...
        layer_list.append(nn.LayerNorm(base_dim))
...
class GNN(nn.Module):
    def __init__(
        ...
        self.conv = nn.ModuleList(
            [CellConv(base_channels, edge_channels) for _ in range(n_layers)]
        )
        self.mlps = nn.ModuleList(
            [
                MLP(2 * base_channels, [2 * base_channels, base_channels])
    ...
    def forward(self, x):
        """
        x: (384, 60, n_base_channels)
        edge_attr: (len(edge), edge_channels)
        edge_index: (2, len(edge))  # これは固定
        """
        assert x.shape[2] == self.base_channels

        edge_index = self.edge_index.to(x.device)
        edge_attr = self.edge_attr.to(x.device)

        # 各頂点ごとに
        x = x.transpose(0, 1).flatten(
            start_dim=0, end_dim=1
        )  # 60, 384　の順番にしないとedgeの番号と対応が合わない
        for conv, mlp in zip(self.conv, self.mlps):
            x_out = conv(x, edge_attr, edge_index)
            x = torch.cat([x, x_out], dim=-1)
            x = mlp(x)
        x = x.view(60, 384, self.base_channels).transpose(0, 1)
        return x
...
class LeapModel(nn.Module):
    def __init__(
        ...
        out_base_channels = same_height_hidden_sizes[-1]
        self.t_head = MLP(
            9 + out_base_channels,
        ...
        self.scaler_head = MLP(
            out_base_channels,
            output_hidden_sizes + [8],
            use_layer_norm=use_output_layer_norm,
        )
    ...
    def forward(self, x, x_cat):
        ...
        class_logits = self.scaler_head(x).mean(dim=1)
