...
class LeapModel(nn.Module):
    def __init__(
        ...
        for _ in range(60):
            layers = []
            now_size = previous_size
            for hidden_size in same_height_hidden_sizes:
                layers.append(nn.Linear(now_size, hidden_size * n_feat_channels))
                layers.append(nn.LayerNorm(hidden_size * n_feat_channels))
                layers.append(nn.LeakyReLU(inplace=True))
                now_size = hidden_size * n_feat_channels
    ...
    def forward(self, x):
        ...
        x = torch.concat(
            [linear(x[:, i, :]).unsqueeze(1) for i, linear in enumerate(self.linears)],
            dim=1,
        )
