...
class Scaler:
    ...
    def scale_input(self, x):
        ...
        q1_log = (
            np.log1p(x[:, 60:120] * 1e9) - self.feat_mean_dict["q1_log"]
        ) / self.feat_std_dict["q1_log"]
        q2_log = (
            np.log1p(x[:, 120:180] * 1e9) - self.feat_mean_dict["q2_log"]
        ) / self.feat_std_dict["q2_log"]
        q3_log = (
            np.log1p(x[:, 180:240] * 1e9) - self.feat_mean_dict["q3_log"]
        ) / self.feat_std_dict["q3_log"]
...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, use_batch_norm=True):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d((2, 1)), DoubleConv(in_channels, out_channels, use_batch_norm)
        )
    ...
    def __init__(self, in_channels, out_channels, bilinear=True, use_batch_norm=True):
        ...
        if bilinear:
            ...
            self.conv = DoubleConv(
                in_channels, out_channels, in_channels // 2, use_batch_norm
            )
        else:
...
class UNet(nn.Module):
    def __init__(
        ...
        self.inc = DoubleConv(n_channels, n_base_channels, use_batch_norm)
...
class LeapModel(nn.Module):
    ...
    def forward(self, x, x_cat):
        ...
        out_q1 = out_q[:, :, 0:1].exp() - out_q[:, :, 1:2].exp()
        out_q2 = out_q[:, :, 2:3].exp() - out_q[:, :, 3:4].exp()
        out_q3 = out_q[:, :, 4:5].exp() - out_q[:, :, 5:6].exp()
