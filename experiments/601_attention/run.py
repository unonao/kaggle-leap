...
class LeapLightningDataModule(LightningDataModule):
    def __init__(
        ...
        self.valid_file_paths = sorted(
            ...
            cfg.exp.train_data_skip_mod // 2 :: cfg.exp.valid_data_skip_mod
        ]  # TODO 変える？
...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.attention_mlp = MLP(
            previous_size * 2,
            [previous_size // 2, previous_size // 4, 1],
    ...
    def forward(self, x, x_cat, x1, x1_cat):
        ...
        score = self.attention_mlp(torch.cat([x, x1], dim=2)).sigmoid()

        x = torch.cat([x, (x - x1) * score], dim=2)
