...
class Scaler:
    ...
    def get_v_scale(self):
        # v の出力のスケールは u に合わせるように学習させるとして、v本来のスケールに戻すための値を返す
        return self.y_rms_sub[240:300] / (self.y_rms_sub[300:360] + self.eps)
...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.wind_v_scaling = wind_v_scaling
        num_embeddings = 60
        ...
        self.pooling = None
        if last_pooling == "avg":
            self.pooling = nn.AdaptiveMaxPool1d(7)
        elif last_pooling == "max":
            self.pooling = nn.AdaptiveAvgPool1d(7)
        elif last_pooling == "linear":
            self.pooling = nn.Linear(same_height_hidden_sizes[-1], 8)
    ...
    def forward(self, x):
        ...
        x = self.pooling(
            x
        )  # (batch, 60, same_height_hidden_sizes[-1]) -> (batch, 60, 7)

        x = x.transpose(-1, -2)  # ([batch, 60, 7] - > [batch, 7, 60])

        # wind用
        x_sin = torch.tanh(x[:, 5:6, :])
        x_cos = torch.tanh(x[:, 6:7, :])

        x = torch.concat(
            [
                x[:, :4, :],
                x[:, 4:5, :] * x_sin,
                x[:, 4:5, :] * x_cos * self.wind_v_scaling,
            ],
            dim=1,
        )
        ...
        triangle_err = x_sin * x_sin + x_cos * x_cos - 1
        return x, (triangle_err * triangle_err).mean()
...
class LeapLightningModule(LightningModule):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.scaler = Scaler(cfg)
        wind_v_scaling = torch.from_numpy(
            self.scaler.get_v_scale().astype(np.float32)
        ).to("cuda")
        self.model = LeapModel(
            **cfg.exp.model,
            seq_feats=cfg.exp.seq_feats,
            scalar_feats=cfg.exp.scalar_feats,
            wind_v_scaling=wind_v_scaling,
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        out, triangle_loss = self.__pred(x, mode)
        out_masked = out[mask]
        y_masked = y[mask]
        loss = self.loss_fc(
            out_masked[:, self.use_cols_index], y_masked[:, self.use_cols_index]
        )
        loss += triangle_loss * self.cfg.exp.triangle_loss_weight
    ...
    def __pred(self, x, mode: str) -> torch.Tensor:
        if (mode == "valid" or mode == "test") and (self.model_ema is not None):
            out, triangle_loss = self.model_ema.module(x)
        else:
            out, triangle_loss = self.model(x)
        return out, triangle_loss
