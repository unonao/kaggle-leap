...
class Scaler:
    ...
    def scale_output(self, y, original_x):
        specific2relative_coef = cal_specific2relative_coef(
            temperature_array=original_x[:, 0:60],
            near_surface_air_pressure=original_x[:, 360],
            hyam=self.hyam,
            hybm=self.hybm,
        )
        y_rh = (
            specific2relative_coef * y[:, 60:120]
            - self.feat_mean_dict["y_relative_humidity_all"]
    ...
    def inv_scale_output(self, y, original_x, post_process=True):
        ...
        y_specific_humidity = (
            y[:, 60:120] * self.feat_std_dict["y_relative_humidity_all"]
            + self.feat_mean_dict["y_relative_humidity_all"]
        ) / specific2relative_coef

        y = y * self.y_rms_sub + self.y_mean
        for i in range(self.y_rms_sub.shape[0]):
            if self.y_rms_sub[i] < self.eps * 1.1:
                y[:, i] = self.y_mean[i]
        y[:, 60:120] = y_specific_humidity
