...
def get_valid_name(cfg):
    return f"{cfg.exp.valid_year_index}_{cfg.exp.valid_data_skip_mod}_6month"
...
class LeapLightningDataModule(LightningDataModule):
    def __init__(
        ...
        for yi in cfg.exp.train_years_index:
            paths = list(glob.glob(str(Path(cfg.exp.dataset_dir) / f"year{yi}_*/*")))
            if yi == cfg.exp.valid_year_index:
                paths = [p for p in paths if sort_key(p) >= (840576 // 4)]
            self.train_file_paths.extend(paths)
        ...
        paths = list(
            glob.glob(
                str(Path(cfg.exp.dataset_dir) / f"year{cfg.exp.valid_year_index}_*/*")
            )
        )
        paths = [p for p in paths if sort_key(p) < (840576 // 4)]
    ...
    class SimDataset(Dataset):
        ...
        def __getitem__(self, index):
            ...
            return (
                ...
                torch.from_numpy(y),
                torch.from_numpy(top1_is_in_bools),
...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.attention_mlp = MLP(
            previous_size * 2,
            [previous_size // 2, previous_size // 4, 5],
    ...
    def forward(self, x, x_cat, x1, x1_cat):
        ...
        attention = self.attention_mlp(torch.cat([x, x1], dim=2))

        out2 = attention.mean(dim=1).sigmoid()

        # class 3,4 は特別（6先と12先)
        x = torch.cat(
            [
                x,
                (x - x1) * attention[:, :, 3:4].sigmoid(),
                (x - x1) * attention[:, :, 4:5].sigmoid(),
            ],
            dim=2,
        )
...
class LeapLightningModule(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        mode = "train"
        x, x_cat, x1, x1_cat, y, top1_is_in_bools, mask, _, _ = batch
        x, x_cat, x1, x1_cat, y, top1_is_in_bools, mask = (
    ...
    def validation_step(self, batch, batch_idx):
        mode = "valid"
        x, x_cat, x1, x1_cat, y, _, mask, original_x, original_y = batch
        ...
        x_cat = x_cat.to(torch.long)
        x1_cat = x1_cat.to(torch.long)
        out, _ = self.__pred(x, x_cat, x1, x1_cat, mode)
        out_masked = out[mask]
        y_masked = y[mask]
        loss = self.loss_fc(
            out_masked[:, self.use_cols_index], y_masked[:, self.use_cols_index]
    ...
    def predict_step(self, batch, batch_idx):
        ...
        x_cat = x_cat.to(torch.long)
        out, _ = self.__pred(x, x_cat, mode)
        return out

    def __pred(self, x, x_cat, x1, x1_cat, mode: str) -> torch.Tensor:
        if (mode == "valid" or mode == "test") and (self.model_ema is not None):
            out, out2 = self.model_ema.module(x, x_cat, x1, x1_cat)
        else:
            out, out2 = self.model(x, x_cat, x1, x1_cat)
        return out, out2
