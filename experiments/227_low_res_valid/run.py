...
def get_valid_name(cfg):
    return f"8h_{cfg.exp.valid_start[0]:02d}-{cfg.exp.valid_start[1]:02d}_{cfg.exp.valid_end[0]:02d}-{cfg.exp.valid_end[1]:02d}_{cfg.exp.valid_data_skip_mod}"
...
def get_train_pattern(cfg, start_year, start_month, end_year, end_month):
    dir_paths = []
...
def get_valid_pattern(cfg, start_year, start_month, end_year, end_month):
    dir_paths = []
    for year in range(start_year, end_year + 1):
        for month in range(1, 13):
            if (year == start_year and month < start_month) or (
                ...
                continue
            dir_paths += [
                Path(cfg.exp.valid_dataset_dir) / f"{year:04d}-{month:02d}/*-{s}.npy"
                for s in ["00000", "28800", "57600"]
            ]
    return dir_paths
...
class LeapLightningDataModule(LightningDataModule):
    def __init__(
        ...
        dir_paths += get_train_pattern(
            cfg, start_year, start_month, end_year, end_month
        )
        ...
        start_year, start_month = cfg.exp.valid_start
        end_year, end_month = cfg.exp.valid_end
        dir_paths = get_valid_pattern(cfg, start_year, start_month, end_year, end_month)
        self.valid_file_paths = sorted(
            list(itertools.chain(*[glob.glob(str(p)) for p in dir_paths]))
        )[:: cfg.exp.valid_data_skip_mod]
        print(f"valid_file_paths: {len(self.valid_file_paths)}")
        ...
        self.valid_dataset = self.TrainDataset(
            self.cfg,
            self.scaler,
    ...
    def val_dataloader(self):
        dataloader = self.val2_dataloader()
        return dataloader
