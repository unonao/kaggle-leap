...
class Scaler:
    ...
    def get_explicit_scaler(self):
        zero_index = self.weight_array == 0
        scale = 1.0 / (self.weight_array * self.y_rms_sub)
        scale[zero_index] = 0
        return scale
    ...
    def scale_output(self, y):
        if self.cfg.exp.explicit_output_scaling:
            y /= self.y_rms_sub
        else:
...
class LeapModel(nn.Module):
    def __init__(
        ...
        seq_feats=[],
        scalar_feats=[],
        explicit_output_scaling=None,
    ):
        super().__init__()
        self.explicit_output_scaling = explicit_output_scaling
    ...
    def forward(self, x):
        ...
        if self.explicit_output_scaling is not None:
            x = x.to(torch.float64) * self.explicit_output_scaling.to(torch.float64)
...
class LeapLightningModule(LightningModule):
    def __init__(self, cfg):
        ...
        explicit_output_scaling = None
        if cfg.exp.explicit_output_scaling:
            explicit_output_scaling = torch.from_numpy(
                self.scaler.get_explicit_scaler()
            ).to("cuda")
        self.model = LeapModel(
            **cfg.exp.model,
            seq_feats=cfg.exp.seq_feats,
            scalar_feats=cfg.exp.scalar_feats,
            explicit_output_scaling=explicit_output_scaling,
        )
