...
class GlobalConv3d(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, conv in enumerate(self.conv_list):
            x_mid = conv(h)
            if (i != 0) and (i != len(self.conv_list) - 1):
                h += x_mid
            else:
                h = x_mid
        return h
...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.global_conv = GlobalConv3d(
            in_channels=n_base_channels,
            out_channels=g_mid_channels,
            mid_channels=g_mid_channels,
    ...
    def forward(self, x, x_cat):
        ...
        2. global な処理
        """
        # batch を戻す -> チャンネルの位置を変更 -> 384を 24x16 に変更
        x = x.view(-1, 384, self.n_base_channels, 60)
        x = x.transpose(1, 2)  # -> (batch, n_base_channels, 384, 60)
        x = x.reshape(-1, self.n_base_channels, 24, 16, 60)
        x = self.global_conv(x)

        # (batch, n_base_channels, 24, 16, 60) -> (batch*384, 60, n_base_channels)
        x = x.flatten(start_dim=2, end_dim=3)  # -> (batch, n_base_channels, 384, 60)
        x = x.transpose(1, 2)  # -> (batch, 384, n_base_channels, 60)
