...
class Scaler:
    ...
    def filter_and_scale(self, x, y):
        ...
        y, y_subtask = self.scale_output(x, y)
        x, x_cat = self.scale_input(x)
        return x, x_cat, y, y_subtask, filter_bool
...
class LeapLightningDataModule(LightningDataModule):
    ...
    class Val2Dataset(Dataset):
        ...
        def __getitem__(self, index):
            ...
            x, x_cat, y, y_subtask, filter_bool = self.scaler.filter_and_scale(
    ...
    def _make_dataset(self, mode="train"):
        ...
        def _process(source):
            for sample in source:
                ...
                x, x_cat, y, y_subtask, mask = self.scaler.filter_and_scale(
                    original_x, original_y
                )
                x, x_cat, y, y_subtask, mask, original_x, original_y = (
                    torch.tensor(x),
                    torch.tensor(x_cat),
                    torch.tensor(y),
                    torch.tensor(y_subtask),
                    torch.tensor(mask),
                    torch.tensor(original_x),
                    torch.tensor(original_y),
                )
                yield x, x_cat, y, y_subtask, mask, original_x, original_y
...
class LeapModel(nn.Module):
    ...
    def forward(self, x, x_cat):
        ...
        out = self.head(x)  # -> (batch, 60, 1)

        out_t = out[:, :, 0:1].exp() - out[:, :, 1:2].exp()
        out_q1 = out[:, :, 2:3].exp() - out[:, :, 3:4].exp()
        out_q2 = out[:, :, 4:5].exp() - out[:, :, 5:6].exp()
        ...
        subout = subout.transpose(-1, -2)
        subout = subout.flatten(start_dim=1)
        out = torch.cat([out, subout], dim=1)
...
class LeapLightningModule(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        out_masked = out[mask]
        y_masked = y[mask]
        y_subtask_masked = y_subtask[mask]

        loss = self.loss_fc(
            out_masked[:, :368][:, self.use_cols_index],
            y_masked[:, self.use_cols_index],
        ) + self.loss_fc(out_masked[:, 368:], y_subtask_masked)
        self.log(
            f"{mode}_loss",
            loss.detach().item(),
            on_step=True,
    ...
    def validation_step(self, batch, batch_idx):
        mode = "valid"
        x, x_cat, y, y_subtask, mask, original_x, original_y = batch
        x, x_cat, y, y_subtask, mask, original_x, original_y = (
            torch.flatten(x, start_dim=0, end_dim=1),
            torch.flatten(x_cat, start_dim=0, end_dim=1),
            torch.flatten(y, start_dim=0, end_dim=1),
        ...
        self.valid_preds.append(out[:, :368].cpu().to(torch.float64).numpy())
...
def predict_val2(cfg: DictConfig, output_path: Path) -> None:
    ...
    model = model.to("cuda")
    model.eval()
    for x, x_cat, _, _, _, original_x, original_y in tqdm(dataloader):
        ...
        preds.append(out[:, :368].cpu().to(torch.float64))
        sub_preds.append(out[:, 368:].cpu().to(torch.float64))
    ...
    with utils.trace("save predict"):
        ...
        sub_preds = Scaler(cfg).inv_scale_output_sub(np.concatenate(sub_preds, axis=0))
        ...
        original_sub_predict_df = pd.DataFrame(
            sub_preds, columns=[i for i in range(sub_preds.shape[1])]
        ).reset_index()
        original_sub_predict_df.to_parquet(output_path / "val2_sub_predict.parquet")
        del original_sub_predict_df
        gc.collect()
...
def predict_test(cfg: DictConfig, output_path: Path) -> None:
    ...
    for x, x_cat, original_x in tqdm(dataloader):
        ...
        preds.append(out[:, :368].detach().cpu().numpy())
