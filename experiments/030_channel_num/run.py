...
class UNet(nn.Module):
    def __init__(
        ...
        self.down3 = Down(4 * n_base_channels, 8 * n_base_channels // factor)
        self.up1 = Up(8 * n_base_channels, 4 * n_base_channels // factor, bilinear)
        self.up2 = Up(4 * n_base_channels, 2 * n_base_channels // factor, bilinear)
        self.up3 = Up(2 * n_base_channels, n_base_channels, bilinear)
        self.outc = OutConv(n_base_channels, n_classes)

        self.class_head = ClassificationHead(
            8 * n_base_channels // factor, n_class_head, pooling="avg", dropout=dropout
        )
