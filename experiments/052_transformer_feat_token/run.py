...
class LeapModel(nn.Module):
    def __init__(
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=60, nhead=1, dim_feedforward=2048, dropout=0.1
        )
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=8)

        self.class_head = nn.Linear(60 * previous_size, 8, bias=True)
    ...
    def forward(self, x):
        ...
        # (batch, 60, 25+embedding_dim) -> (batch, 60, dim)
        x = self.same_height_encoder(x)

        x = self.encoder(x.transpose(1, 2)).transpose(1, 2)

        class_logits = self.class_head(x.flatten(start_dim=1))

        # (batch, 60, dim) -> (batch, 60, dim)
        x = self.pooling(x)
