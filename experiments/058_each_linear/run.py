...
class LeapModel(nn.Module):
    def __init__(
        ...
        scaler_num = 16 + len(scalar_feats)
        self.scaler_weights = nn.Parameter(torch.randn(60, scaler_num))
        self.scaler_biases = nn.Parameter(torch.randn(60, scaler_num))
    ...
    def forward(self, x):
        ...
        for i, _ in enumerate(self.scalar_feats):
            ...
            x_scalar = x[:, start:end]
            scaler_list.append(x_scalar)
        scaler_input = torch.cat(scaler_list, dim=1)
        scaler_input = (
            torch.einsum("bs,hs->bhs", scaler_input, self.scaler_weights)
            + self.scaler_biases
        )

        input_list.append(scaler_input)
