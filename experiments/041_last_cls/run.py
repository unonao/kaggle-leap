...
from model import SqueezeformerEncoder
...
class Scaler:
    def __init__(self, cfg, device="cpu"):
        ...
        self.x_mean = torch.tensor(
            np.load(Path(cfg.exp.scale_dir) / f"x_mean_{cfg.exp.norm_name}.npy")
        ).to(device)
        ...
        self.y_mean = torch.tensor(
            np.load(Path(cfg.exp.scale_dir) / f"y_nanmean_{cfg.exp.norm_name}.npy")
        ).to(device)
        self.y_rms_sub = torch.tensor(
            np.maximum(
                np.load(Path(cfg.exp.scale_dir) / f"y_rms_sub_{cfg.exp.norm_name}.npy"),
                self.eps,
            )
        ).to(device)
        self.y_lower_bound = torch.tensor(
            np.load(Path(cfg.exp.scale_dir) / f"y_lower_bound_{cfg.exp.norm_name}.npy")
        ).to(device)
        self.y_upper_bound = torch.tensor(
            np.load(Path(cfg.exp.scale_dir) / f"y_upper_bound_{cfg.exp.norm_name}.npy")
        ).to(device)
    ...
    def filter_and_scale(self, x, y):
        ...
        filter_bool = torch.all(y >= self.y_lower_bound - self.eps, axis=1) & torch.all(
...
class LeapLightningDataModule(LightningDataModule):
    ...
    class TestDataset(Dataset):
        ...
        def __getitem__(self, index):
            original_x = torch.from_numpy(self.x[index])
            return self.scaler.scale_input(original_x)
    ...
    def _make_dataset(self, mode="train"):
        ...
        def _process(source):
            for sample in source:
                ...
                yield x, y, mask, original_x, original_y
...
class CustomLoss(nn.Module):
    def __init__(self, cfg, device="cuda"):
        ...
        self.cfg = cfg
        self.scaler = Scaler(cfg, device=device)
        self.water_conservation_loss_rate = cfg.exp.loss.water_conservation_loss_rate
        ...
        self.hyai = torch.tensor(grid_info["hyai"].to_numpy()).to(device)
        self.hybi = torch.tensor(grid_info["hybi"].to_numpy()).to(device)

        self.mse_loss = nn.MSELoss()
        # self.mae_loss = nn.L1Loss()

    def cal_pressures(self, state_ps):
        pressures = self.hyai * P0 + self.hybi[None, :] * state_ps[:, None]
        pressures = torch.diff(pressures, n=1)
        return pressures

    def cal_water_diff(self, original_input, original_output):
        """
        秒間の水分量の変化を計算する。inputの水分量と比較する際は1200倍してスケールを合わせる必要がある
        1200倍すると大体 最大・最小がプラスマイナス1程度になることは確認
        """
        state_ps = original_input[:, 360]
        ptend_q0001 = original_output[:, 60:120]
        ptend_q0002 = original_output[:, 120:180]
        ptend_p0003 = original_output[:, 180:240]
        cam_out_PRECC = original_output[:, 363]
        pressures = self.cal_pressures(state_ps)
        total_water_errr = (
            (ptend_q0001 + ptend_q0002 + ptend_p0003) * pressures / G
        ).sum(axis=1)
        surface_water_flux = -1e3 * cam_out_PRECC
        water_diff = total_water_errr - surface_water_flux
        return water_diff

    def forward(self, output, target, original_input, original_target):
        loss = self.mse_loss(
            output[:, self.use_cols_index], target[:, self.use_cols_index]
        )

        if self.water_conservation_loss_rate:
            original_output = self.scaler.inv_scale_output(output)
            water_diff_output = 1200.0 * self.cal_water_diff(
                original_input, original_output
            )
            water_diff_target = 1200.0 * self.cal_water_diff(
                original_input, original_target
            )
            water_conservation_loss = self.mse_loss(
                water_diff_output, water_diff_target
            )

            loss += water_conservation_loss * self.water_conservation_loss_rate
...
class LeapModel(nn.Module):
    def __init__(
        self,
        same_height_hidden_sizes=[96, 96],
        embedding_dim=10,
        last_pooling="avg",
        encoder_params={
            "encoder_dim": 96,
        ...
        self.cls_embedding = nn.Embedding(1, previous_size)
        ...
        self.encoder = SqueezeformerEncoder(**encoder_params)

        self.header = nn.Sequential(
            *[
                nn.Linear(previous_size, 8),
            ]
        )
    ...
    def forward(self, x):
        ...
        x_cls = self.cls_embedding(torch.zeros(x.shape[0], 1).long().to(x.device))
        x = torch.cat([x, x_cls], dim=1)

        # (batch, 61, 25+embedding_dim) -> (batch, 10, same_height_hidden_sizes[-1])
        x = self.same_height_encoder(x)

        x = self.encoder(x)
        class_logits = self.header(x[:, 60, :])

        # (batch, 60, same_height_hidden_sizes[-1]) -> (batch, 60, 6)
        x = self.pooling(x[:, :60, :])
        # ([batch, 6, 60, 1] - > [batch, 1, 6, 60])
        x = x.transpose(-1, -2)
...
class LeapLightningModule(LightningModule):
    def __init__(self, cfg):
        ...
        self.scaler = Scaler(cfg)
        self.loss_fc = CustomLoss(cfg, "cuda")
        self.model_ema = None
    ...
    def training_step(self, batch, batch_idx):
        mode = "train"
        x, y, mask, original_x, original_y = batch
        x, y, mask, original_x, original_y = (
        ...
        x, y, original_x, original_y = (
            ...
            original_x.to(self.torch_dtype),
            original_y.to(self.torch_dtype),
        )
        out = self.__pred(x, mode)
        loss = self.loss_fc(out[mask], y[mask], original_x[mask], original_y[mask])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        out = self.__pred(x, mode)
        loss = self.loss_fc(out, y, original_x, original_y)
        self.log(
        ...
        self.valid_preds.append(out.cpu().to(torch.float64))
        self.valid_labels.append(original_y.cpu().to(torch.float64))
        return loss
    ...
    def on_validation_epoch_end(self):
        valid_preds = torch.concat(self.valid_preds, dim=0)
        valid_preds = self.scaler.inv_scale_output(valid_preds).numpy()
        valid_labels = torch.concat(self.valid_labels, dim=0).numpy()
        r2_scores = r2_score(
...
def predict_valid(cfg: DictConfig, output_path: Path) -> None:
    ...
    model = model.to("cuda")
    model.eval()
    for x, _, _, _, original_y in tqdm(dataloader):
    ...
    with utils.trace("save predict"):
        _preds = torch.cat(preds)
        preds = Scaler(cfg).inv_scale_output(_preds).numpy()
...
def predict_test(cfg: DictConfig, output_path: Path) -> None:
    ...
    preds = Scaler(cfg).inv_scale_output(preds).numpy()
