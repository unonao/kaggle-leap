...
class LeapLightningDataModule(LightningDataModule):
    ...
    class TestDataset(Dataset):
        def __init__(self, cfg, test_df, scaler):
            ...
            self.x_location_category = (
                test_df[:, 0].str.extract(r"test_(\d+)").str.to_integer().to_numpy()
                % 384
            )
        ...
        def __getitem__(self, index):
            return (
                torch.from_numpy(self.scaler.scale_input(self.x[index])),
                torch.tensor(self.x_location_category[index], dtype=torch.long),
            )
    ...
    def _make_dataset(self, mode="train"):
        ...
        def _process(source):
            for sample in source:
                ...
                # index をカテゴリ変数として追加
                x_location_category = torch.arange(x.shape[0], dtype=torch.long)
                yield x, x_location_category, y, mask, original_y
...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.location_embedding = nn.Embedding(384, location_embedding_dim)
        self.constant16_encoder = nn.Linear(1, 60)

        layers = []
        previous_size = 9 + 16 + positional_embedding_dim + location_embedding_dim
    ...
    def forward(self, x, x_location_category):
        ...
        x_location = self.location_embedding(
            x_location_category.unsqueeze(-1).repeat(1, 60)
        )
...
class LeapLightningModule(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        mode = "train"
        x, x_loc, y, mask, _ = batch
        x, x_loc, y, mask = (
            torch.flatten(x, start_dim=0, end_dim=1),
            torch.flatten(x_loc, start_dim=0, end_dim=1),
    ...
    def validation_step(self, batch, batch_idx):
        mode = "valid"
        x, x_loc, y, mask, original_y = batch
        x, x_loc, y, mask, original_y = (
            torch.flatten(x, start_dim=0, end_dim=1),
            torch.flatten(x_loc, start_dim=0, end_dim=1),
            torch.flatten(y, start_dim=0, end_dim=1),
            torch.flatten(mask, start_dim=0, end_dim=1),
    ...
    def __pred(self, x, x_loc, mode: str) -> torch.Tensor:
        if (mode == "valid" or mode == "test") and (self.model_ema is not None):
            out = self.model_ema.module(x, x_loc)
        else:
            out = self.model(x, x_loc)
        return out
...
def predict_valid(cfg: DictConfig, output_path: Path) -> None:
    ...
    model = model.to("cuda")
    model.eval()
    for x, x_loc, _, _, original_y in tqdm(dataloader):
        x, x_loc, original_y = x.to("cuda"), x_loc.to("cuda"), original_y.to("cuda")

        x = torch.flatten(x, start_dim=0, end_dim=1)
        x_loc = torch.flatten(x_loc, start_dim=0, end_dim=1)
        original_y = torch.flatten(original_y, start_dim=0, end_dim=1)
        with torch.no_grad():
            out = model(x.to(torch_dtype), x_loc)
...
def predict_test(cfg: DictConfig, output_path: Path) -> None:
    ...
    model = model.to("cuda")
    model.eval()
    for x, x_loc in tqdm(dataloader):
        x, x_loc = x.to("cuda"), x_loc.to("cuda")
