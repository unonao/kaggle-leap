...
class Scaler:
    ...
    def process_features(self, x_array):
        ...
        if "q1_log" in self.cfg.exp.seq_feats:
            q1_log = np.log1p(x_array[:, 60:120] * 1e9)
            feats.append(q1_log)
        if "q2_log" in self.cfg.exp.seq_feats:
            q2_log = np.log1p(x_array[:, 120:180] * 1e9)
            feats.append(q2_log)
        if "q3_log" in self.cfg.exp.seq_feats:
            q3_log = np.log1p(x_array[:, 180:240] * 1e9)
            feats.append(q3_log)
        if "cloud_watter_log" in self.cfg.exp.seq_feats:
            cloud_watter_log = np.log1p(
                (x_array[:, 120:180] + x_array[:, 180:240]) * 1e9
            )
            feats.append(cloud_watter_log)
...
class MultiHeadMLP(nn.Module):
    def __init__(self, in_size, hidden_sizes, head=5):
    ...
    def forward(self, x):
        # x を head だけ分割
        x = x.chunk(self.head, dim=1)
        out = []
        for i in range(self.head):
            out.append(self.mlps[i](x[i]))
...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.same_height_encoder = MultiHeadMLP(
            previous_size, same_height_hidden_sizes, head=head
        )
        ...
        self.t_head = MultiHeadMLP(
            8 + n_base_channels, same_height_hidden_sizes + [1], head=head
        )
        self.q_head = MultiHeadMLP(
            9 + n_base_channels, same_height_hidden_sizes + [3], head=head
        )
