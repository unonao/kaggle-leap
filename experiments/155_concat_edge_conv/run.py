...
class Height60Conv(MessagePassing):
    ...
    def __init__(self, base_channels, edge_channels, kernel_sizes=[1], bias=False):
        ...
        input_channels = 2 * base_channels
        adjacency_conv_list = []
        for kernel_size in kernel_sizes:
            ...
            adjacency_conv_list.append(nn.ReLU(inplace=True))
            input_channels = base_channels
        self.adjacency_conv = nn.Sequential(*adjacency_conv_list)

        self.node_conv = nn.Conv1d(
            2 * base_channels, base_channels, kernel_size=1, padding="same", bias=bias
        )
    ...
    def forward(self, x, edge_attr, edge_index):
        ...
        x = x.view(
            -1, 2 * self.base_channels, 60
        )  # (384, 2*n_base_channels*60) -> (384,2* n_base_channels, 60)
        x = self.node_conv(x)
        return x
    ...
    def message(self, x_i, x_j, edge_attr):
        ...
        h = torch.concat([h_j - h_i, edge_attr.unsqueeze(-1).repeat(1, 1, 60)], dim=-2)
        h = self.adjacency_conv(h)
        h = h.view(-1, self.base_channels * 60)
        return h
...
class LocalConv(nn.Module):
    def __init__(
        ...
        for kernel_size in kernel_sizes:
            ...
            module_list.append(nn.LayerNorm((mid_channels, 60)))
            module_list.append(nn.ReLU(inplace=True))
        self.local_conv = nn.Sequential(*module_list)
...
class GNN(nn.Module):
    def __init__(
        ...
        self.local_conv = nn.ModuleList(
            [
                LocalConv(base_channels, kernel_sizes=local_kernel_sizes)
                for _ in range(n_layers)
            ]
        )
        self.gnn_conv = nn.ModuleList(
            [
                Height60Conv(base_channels, edge_channels, gnn_kernel_sizes, bias=bias)
                for _ in range(n_layers)
            ]
        )
        self.outc = DoubleConv(base_channels, out_channels)
    ...
    def forward(self, x):
        ...
        x = self.inc(x)
        for local_conv, gnn_conv in zip(self.local_conv, self.gnn_conv):
            x = local_conv(x)
            x = gnn_conv(x, edge_attr, edge_index)
        x = self.outc(x)
...
class LeapModel(nn.Module):
    def __init__(
        ...
        out_base_channels = n_base_channels
        self.head = MLP(
            out_base_channels,
            output_hidden_sizes + [6 + 8],  # 6*60+8 次元
    ...
    def forward(self, x, x_cat):
        ...
        3. 各地域ごとに出力を作成
        """
        out = self.head(x)  # -> (batch, 60, 6+8)

        class_logits = out[:, :, 6:].mean(dim=1)
        out = out[:, :, :6].transpose(-1, -2).flatten(start_dim=1)
