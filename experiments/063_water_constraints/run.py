    ...
    class TestDataset(Dataset):
        ...
        def __getitem__(self, index):
            return torch.from_numpy(
                self.scaler.scale_input_1d(self.x[index])
...
def predict_valid(cfg: DictConfig, output_path: Path) -> None:
    ...
    model = model.to("cuda")
    model.eval()
    for x, _, _, original_x, original_y in tqdm(dataloader):
        ...
        original_xs.append(original_x.cpu())
    ...
    with utils.trace("save predict"):
        ...
        # water constraints
        if cfg.exp.water_constraints:
            print(preds.shape, original_xs.shape)
            preds[:, 60:240] = np.clip(
                preds[:, 60:240], (-original_xs[:, 60:240] / 1200.0), None
            )
...
def predict_test(cfg: DictConfig, output_path: Path) -> None:
    ...
    model = model.to("cuda")
    model.eval()
    for x, original_x in tqdm(dataloader):
        x = x.to("cuda")
