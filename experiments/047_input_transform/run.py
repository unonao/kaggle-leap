...
class Scaler:
    def __init__(self, cfg):
        ...
        self.x_mean = np.load(Path(cfg.exp.input_transform_dir) / "x_mean.npy")
        self.power_transformer = pickle.load(
            (Path(cfg.exp.input_transform_dir) / "yeo-johnson.pkl").open("rb")
        )
...
class LeapLightningDataModule(LightningDataModule):
    ...
    class TestDataset(Dataset):
        def __init__(self, cfg, test_df, scaler):
            ...
            self.x = self.scaler.scale_input(test_df[:, 1:].to_numpy())
...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.cnn = CNN(F1=368 // 2, D=2, final_dropout=final_dropout)
    ...
    def forward(self, x):
        ...
        x = x.transpose(
            ...
            1
        )  # (batch, 60, same_height_hidden_sizes[-1]) -> (batch, 1, same_height_hidden_sizes[-1], 60)

        x = self.cnn(
            x
        )  # (batch, same_height_hidden_sizes[-1], 60) -> (batch, 368, *, *)

        x = self.pooling(x)  # -> (batch, 368, 1, 1)
