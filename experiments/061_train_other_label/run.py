...
class LeapLightningModule(LightningModule):
    def __init__(self, cfg):
        ...
        self.train_unuse_cols_index = sorted(
            list(
                set(cfg.exp.additional_replace_target + cfg.cols.weight_zero_index_list)
            )
        )
        self.train_unuse_cols_index = torch.tensor(
            [i for i in range(368) if i not in self.train_unuse_cols_index]
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_masked = y[mask]
        loss = self.loss_fc(
            out_masked[:, self.train_unuse_cols_index],
            y_masked[:, self.train_unuse_cols_index],
        )
