...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.lat_lon_tensor = get_lat_lon_tensor()  # (384, 3)
        ...
        previous_size = (
            9 + 16 + 3 + embedding_dim
        )  # 9: sequence, 16: scalar, 3: lat_lon
        ...
        self.global_transformer = TransformerEncoderBlock(
            n_dim=g_dim * 60,
            nhead=g_nhead,
    ...
    def forward(self, x, x_cat):
        ...
        x_lat_lon = self.lat_lon_tensor.to(x.device)
        x_lat_lon = x_lat_lon.repeat(batch, 1, 1)  # (batch, 384, 3)
        x_lat_lon_flatten = x_lat_lon.flatten(start_dim=0, end_dim=1)  # (batch*384, 3)
        ...
        x_lat_lon_flatten = x_lat_lon_flatten.unsqueeze(-1)

        x_lat_lon_flatten = self.constant_encoder(x_lat_lon_flatten).transpose(1, 2)
        ...
        # (batch*384, 60, dim) -> (batch*384, 60, same_height_hidden_sizes[-1]*n_feat_channels)
        ...
        # batch を戻して処理
        x = x.view(
            -1, 384, self.n_base_channels, 60
        )  # -> (batch, 384, n_base_channels, 60)

        h = x[:, :, : self.g_dim, :]
        h = h.flatten(start_dim=2, end_dim=3)
        h = self.global_transformer(h)
        h = h.view(-1, 384, self.g_dim, 60)
        x = torch.cat([h, x[:, :, self.g_dim :, :]], dim=2)
        x = x.flatten(start_dim=0, end_dim=1)  # ->(batch*384, n_base_channels, 60)
