...
class Scaler:
    ...
    def process_features(self, x_array):
        ...
        if "cloud_watter_log" in self.cfg.exp.seq_feats:
            cloud_water_log_array = np.log1p(
                (x_array[:, 120:180] + x_array[:, 180:240]) * 1e9
            )
            feats.append(cloud_water_log_array)
    ...
    def scale_input(self, x):
        ...
        x[:, 120:180] = (x[:, 120:180]) / self.feat_std_dict["cloud_water"]
        x[:, 180:240] = (x[:, 180:240]) / self.feat_std_dict["cloud_water"]
    ...
    def get_q23_scale(self):
        # 出力のスケール変換に使う値
        q2_scale = self.feat_std_dict["cloud_water"] / (
            1200.0 * self.y_rms_sub[120:180]
        )
        q3_scale = self.feat_std_dict["cloud_water"] / (
            1200.0 * self.y_rms_sub[180:240]
        )
        return q2_scale, q3_scale
...
class MLP(nn.Module):
    def __init__(self, in_size, hidden_sizes, use_layer_norm=False, exponential=False):
        ...
        self.last_layer = nn.Linear(previous_size, 1)

    def forward(self, x):
        x = self.mlp(x)
        if self.exponential:
            x = self.last_layer(x.exp())
        return x
...
class LeapModel(nn.Module):
    def __init__(
        ...
        dropout=0.2,
        n_base_channels=32,
        cw_exponential=False,
        device="cuda",
    ):
        super().__init__()
        self.device = device
        self.q2_scale = q2_scale.unsqueeze(0).unsqueeze(-1).to(self.device)
        self.q3_scale = q3_scale.unsqueeze(0).unsqueeze(-1).to(self.device)
        ...
        self.t_head = MLP(
            8 + n_base_channels,
            same_height_hidden_sizes + [1],
            use_layer_norm=use_output_layer_norm,
        )
        self.q1_head = MLP(
            4 + n_base_channels,
            same_height_hidden_sizes + [2],
            use_layer_norm=use_output_layer_norm,
        )
    ...
    def forward(self, x, x_cat):
        ...
        out_q2 = (out_q23_ratio[:, :, 0:1] * out_cw - x_state_q0002) * self.q2_scale
        out_q3 = (out_q23_ratio[:, :, 1:2] * out_cw - x_state_q0003) * self.q3_scale
        # out_q23_ratio.shape=torch.Size([384, 60, 2]) out_cw.shape=torch.Size([384, 60, 1]) x_state_q0002.shape=torch.Size([384, 60, 1]), self.q2_scale.shape=torch.Size([1, 60, 1]), out_q2.shape=torch.Size([384, 60, 1]), out_q3.shape=torch.Size([384, 60, 1])
        """
        for h in range(60):
            print("h:", h)
            print(
                f"{out_q23_ratio[0, h, 0].item()=}, {out_q23_ratio[0, h, 1].item()=}, {out_cw[0, h].item()=}, "
            )
            print(
                f"{x_state_q0002[0, h].item()=}, {self.q2_scale[0, h].item()=}, {out_q2[0, h].item()=}"
            )
            print(
                f"{x_state_q0003[0, h].item()=}, {self.q3_scale[0, h].item()=}, {out_q3[0, h].item()=}"
            )
        print()
        """
...
class LeapLightningModule(LightningModule):
    def __init__(self, cfg):
        ...
        self.scaler = Scaler(cfg)
        q2_scale, q3_scale = self.scaler.get_q23_scale()
        self.q2_scale = torch.tensor(q2_scale, dtype=torch.float32)
        self.q3_scale = torch.tensor(q3_scale, dtype=torch.float32)
        self.model = LeapModel(
            q2_scale=self.q2_scale,
            q3_scale=self.q3_scale,
