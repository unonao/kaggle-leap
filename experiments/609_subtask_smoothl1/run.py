...
class DiffClassifier(nn.Sequential):
    def __init__(self, in_channels, out_nums, pooling="avg", dropout=0.2):
        if pooling not in ("max", "avg"):
            raise ValueError(
                "Pooling should be one of ('max', 'avg'), got {}.".format(pooling)
...
class LeapLightningModule(LightningModule):
    def __init__(self, cfg):
        ...
        self.sub_loss_fc = nn.CrossEntropyLoss()
    ...
    def training_step(self, batch, batch_idx):
        mode = "train"
        x, x_cat, x1, x1_cat, y, sub_target, mask, _, _ = batch
        x, x_cat, x1, x1_cat, y, sub_target, mask = (
            ...
            torch.flatten(sub_target, start_dim=0, end_dim=1),
        ...
        loss += self.sub_loss_fc(out2, sub_target) * self.cfg.exp.sub_loss_weight
