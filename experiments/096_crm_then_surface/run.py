...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.unet = UNet(
            ...
            bottleneck_out_nums=bottleneck_out_nums,
            bilinear=bilinear,
            dropout=dropout,
            n_base_channels=n_base_channels,
            use_batch_norm=use_batch_norm,
        )
        self.unet_radiation = UNet(
            n_channels=n_base_channels + radiation_hidden_sizes[-1],
            n_classes=n_base_channels,
            bottleneck_out_nums=bottleneck_out_nums,
            bilinear=bilinear,
            dropout=dropout,
    ...
    def forward(self, x, x_cat):
        ...
        x_cloud = self.same_height_encoder(x_cloud)
        ...
        coupling
         surfaceの情報を結合
        """
        # (batch, 61, same_height_hidden_sizes[-1])
        x_cloud = torch.cat([x_cloud, x_surface.unsqueeze(1)], dim=1)

        """
        CRM(cloud resolving model)
            x_cloud: -> (batch, 61, n_base_channels)
            bottleneck_feat: -> (batch, bottleneck_out_nums)
        """
        x_cloud = x_cloud.transpose(-1, -2).unsqueeze(
            -1
        )  # -> (batch, n_base_channels, 61, 1)
        x_cloud, bottleneck_feat = self.unet(x_cloud)
        x_cloud = x_cloud.squeeze(-1)  # ->(batch, n_base_channels, 61)
        ...
        x = torch.cat(
            [
                x_cloud[:, :, :60],
                x_radiation.unsqueeze(-1).repeat(1, 1, 60),
            ],
            dim=1,
        )  # -> (batch, n_base_channels + radiation_hidden_sizes[-1], 60)
        x = x.unsqueeze(-1)
        x, _ = self.unet_radiation(x)
