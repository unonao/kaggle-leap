...
class Scaler:
    ...
    def process_features(self, x_array):
        ...
        feats = [x_array[:, :556]]
        t_pre = np.pad(x_array[:, :59], ((0, 0), (1, 0)), "constant", constant_values=0)
        t_post = np.pad(
            x_array[:, 1:60], ((0, 0), (0, 1)), "constant", constant_values=0
        )
        q1_pre = np.pad(
            x_array[:, 60:119], ((0, 0), (1, 0)), "constant", constant_values=0
        )
        q1_post = np.pad(
            x_array[:, 61:120], ((0, 0), (0, 1)), "constant", constant_values=0
        )
        q2_pre = np.pad(
            x_array[:, 120:179], ((0, 0), (1, 0)), "constant", constant_values=0
        )
        q2_post = np.pad(
            x_array[:, 121:180], ((0, 0), (0, 1)), "constant", constant_values=0
        )
        q3_pre = np.pad(
            x_array[:, 180:239], ((0, 0), (1, 0)), "constant", constant_values=0
        )
        q3_post = np.pad(
            x_array[:, 181:240], ((0, 0), (0, 1)), "constant", constant_values=0
        )
        feats.append(t_pre)
        feats.append(t_post)
        feats.append(q1_pre)
        feats.append(q1_post)
        feats.append(q2_pre)
        feats.append(q2_post)
        feats.append(q3_pre)
        feats.append(q3_post)
    ...
    def scale_input(self, x):
        ...
        for i in range(4):
            ...
            for j in range(2):
                x_start = 556 + 60 * (2 * i + j)
                x_end = 556 + 60 * (2 * i + j + 1)
                zero_index = x_start if j == 0 else x_end - 1
                x[:, x_start:x_end] = (
                    x[:, x_start:x_end] - self.feat_mean_dict["base"][start:end]
                ) / self.feat_std_dict["base"][start:end]
                x[:, zero_index] = 0
        ...
        for i, key in enumerate(self.cfg.exp.scalar_feats):
            start = 556 + (len(self.cfg.exp.seq_feats) + 8) * 60 + i
            end = 556 + (len(self.cfg.exp.seq_feats) + 8) * 60 + i + 1
            x[:, start:end] = (
                x[:, start:end] - self.feat_mean_dict[key]
            ) / self.feat_std_dict[key]
...
class LeapModel(nn.Module):
    def __init__(
        ...
        previous_size = 9 + 16 + 8 + embedding_dim
    ...
    def forward(self, x, x_cat):
        ...
        for i, _ in enumerate(self.scalar_feats):
            start = 556 + (len(self.seq_feats) + 8) * 60 + i
            end = 556 + (len(self.seq_feats) + 8) * 60 + i + 1
