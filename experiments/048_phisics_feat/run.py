...
class Scaler:
    def __init__(self, cfg):
        ...
        self.x_mean = np.load(
            Path(cfg.exp.scale_dir) / f"x_mean_base_{cfg.exp.norm_name}.npy"
        )
        self.x_std = np.maximum(
            np.load(Path(cfg.exp.scale_dir) / f"x_std_base_{cfg.exp.norm_name}.npy"),
            self.eps,
        )
        self.phisics_feat_mean_dict = {
            f"{name}_mean": np.load(
                Path(cfg.exp.scale_dir) / f"x_mean_{name}_{cfg.exp.norm_name}.npy"
            )
            for name in cfg.exp.phisics_feat
        }
        self.phisicis_feat_std_dict = {
            f"{name}_std": np.maximum(
                np.load(
                    Path(cfg.exp.scale_dir) / f"x_std_{name}_{cfg.exp.norm_name}.npy"
                ),
                self.eps,
            )
            for name in cfg.exp.phisics_feat
    ...
    def scale_input(self, x):
        x[:, :556] = (x[:, :556] - self.x_mean) / self.x_std
        if "pressures" in self.cfg.exp.phisics_feat:
            x[:, 556 : 556 + 60] = (
                x[:, 556 : 556 + 60] - self.phisics_feat_mean_dict["pressures_mean"]
            ) / self.phisicis_feat_std_dict["pressures_std"]
        if "water" in self.cfg.exp.phisics_feat:
            x[:, 556 + 60 : 556 + 120] = (
                x[:, 556 + 60 : 556 + 120] - self.phisics_feat_mean_dict["water_mean"]
            ) / self.phisicis_feat_std_dict["water_std"]
        if "energy" in self.cfg.exp.phisics_feat:
            x[:, 556 + 120 : 556 + 180] = (
                x[:, 556 + 120 : 556 + 180] - self.phisics_feat_mean_dict["energy_mean"]
            ) / self.phisicis_feat_std_dict["energy_std"]
...
class LeapLightningDataModule(LightningDataModule):
    ...
    class TestDataset(Dataset):
        def __init__(self, cfg, test_df, scaler, hyai, hybi):
            ...
            if "pressures" in self.cfg.exp.phisics_feat:
                p0 = 1e5
                ps = self.x[:, 360]  # state_ps
                pressures_array = self.hyai * p0 + self.hybi[None, :] * ps[:, None]
                pressures_array = np.diff(pressures_array, n=1)
                self.x = np.concatenate([self.x, pressures_array], axis=1)
            if "water" in self.cfg.exp.phisics_feat:
                sq0001_array = self.x[:, 60:120]
                sq0002_array = self.x[:, 120:180]
                sq0003_array = self.x[:, 180:240]
                water_array = sq0001_array + sq0002_array + sq0003_array
                self.x = np.concatenate([self.x, water_array], axis=1)
            if "energy" in self.cfg.exp.phisics_feat:
                energy_array = pressures_array * water_array
                self.x = np.concatenate([self.x, energy_array], axis=1)
    ...
    def _make_dataset(self, mode="train"):
        ...
        def _process(source):
            for sample in source:
                ...
                if "pressures" in self.cfg.exp.phisics_feat:
                    p0 = 1e5
                    ps = original_x[:, 360]  # state_ps
                    pressures_array = self.hyai * p0 + self.hybi[None, :] * ps[:, None]
                    pressures_array = np.diff(pressures_array, n=1)
                    original_x = np.concatenate([original_x, pressures_array], axis=1)
                if "water" in self.cfg.exp.phisics_feat:
                    sq0001_array = original_x[:, 60:120]
                    sq0002_array = original_x[:, 120:180]
                    sq0003_array = original_x[:, 180:240]
                    water_array = sq0001_array + sq0002_array + sq0003_array
                    original_x = np.concatenate([original_x, water_array], axis=1)
                if "energy" in self.cfg.exp.phisics_feat:
                    energy_array = pressures_array * water_array
                    original_x = np.concatenate([original_x, energy_array], axis=1)
...
class LeapModel(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.phisics_feat = phisics_feat
        ...
        if "pressures" in self.phisics_feat:
            previous_size += 1
        if "water" in self.phisics_feat:
            previous_size += 1
        if "energy" in self.phisics_feat:
            previous_size += 1
    ...
    def forward(self, x):
        ...
        if "pressures" in self.phisics_feat:
            x_pressures = x[:, 556:616].unsqueeze(-1)
            input_list.append(x_pressures)
        if "water" in self.phisics_feat:
            x_water = x[:, 616:676].unsqueeze(-1)
            input_list.append(x_water)
        if "energy" in self.phisics_feat:
            x_energy = x[:, 676:736].unsqueeze(-1)
            input_list.append(x_energy)
...
class LeapLightningModule(LightningModule):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.model = LeapModel(**cfg.exp.model, phisics_feat=cfg.exp.phisics_feat)
        self.scaler = Scaler(cfg)
