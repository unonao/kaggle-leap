...
class LeapModel(nn.Module):
    ...
    def forward(self, x, x_cat):
        ...
        x_first, _ = self.unet_first(x[:, :, : self.split_pos[0], :])
        x_second, _ = self.unet_second(
            x[:, :, self.split_pos[0] : self.split_pos[1], :]
        )
        x_third, _ = self.unet_third(x[:, :, self.split_pos[1] :, :])
        x_out = torch.cat([x_first, x_second, x_third], dim=2)
        x = torch.cat([x, x_out], dim=1)
