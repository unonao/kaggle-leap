...
class Scaler:
    ...
    def filter_and_scale(self, x, y):
        filter_bool = (y < self.cfg.exp.outlier_y_rate) & (
            y > -self.cfg.exp.outlier_y_rate
        )
...
class FilteredMSELoss(nn.Module):
    def __init__(self):
        super(FilteredMSELoss, self).__init__()
        self.mse_loss = nn.MSELoss(
            reduction="none"
        )  # 個々の要素の損失を計算するために 'none' を指定

    def forward(self, input, target, filter=None):
        # MSELossを計算
        loss = self.mse_loss(input, target)

        if filter is not None:
            # フィルターテンソルで損失をフィルタリング
            filtered_loss = loss * filter.float()
            # フィルタリングされた損失の平均を計算
            return filtered_loss.sum() / filter.sum()
        else:
            # フィルターがない場合、通常の損失の平均を計算
            return loss.mean()
...
class LeapLightningModule(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.loss_fc(
            ...
            filter_bool[:, self.use_cols_index],
        )
    ...
    def validation_step(self, batch, batch_idx):
        mode = "valid"
        x, x_cat, y, filter_bool, original_x, original_y = batch
