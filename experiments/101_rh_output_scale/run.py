...
class Scaler:
    ...
    def get_specific2relative_coef(self, x):
        return cal_specific2relative_coef(
    ...
    def get_output_rh_scale(self, x, rh_coef):
        """
        outputに対してのscale。これをかけてからmseを算出させる
        """
        scale = (
            self.feat_std_dict["y_relative_humidity_all"]
            / rh_coef
            / self.y_rms_sub[60:120]
        )
        return scale

    def scale_input_1d(self, x):
        x = x[None, :]
        x, x_cat, x_scale = self.scale_input(x)
        x, x_cat, x_scale = x.reshape(-1), x_cat.reshape(-1), x_scale.reshape(-1)
        return x, x_cat, x_scale
    ...
    def scale_input(self, x):
        ...
        rh_coef = self.get_specific2relative_coef(x)
        x_scale = self.get_output_rh_scale(x, rh_coef)

        x_rh = rh_coef * x[:, 60:120]
    ...
    def scale_output(self, y, original_x):
        y_sh = (
            y[:, 60:120] / self.y_rms_sub[60:120]
        )  # target は分散だけを整えて平均値で引かない
        y = (y - self.y_mean) / self.y_rms_sub
        y[:, 60:120] = y_sh
        return y

    def inv_scale_output(self, y, original_x, post_process=True):
        y_sh = y[:, 60:120] * self.y_rms_sub[60:120]
    ...
    def filter_and_scale(self, x, y):
        ...
        y = self.scale_output(y, x)
        if x.ndim == 1:
            x, x_cat, x_scale = self.scale_input_1d(x)
        else:
            x, x_cat, x_scale = self.scale_input(x)
        return x, x_cat, x_scale, y, filter_bool
...
class LeapLightningDataModule(LightningDataModule):
    ...
    class TestDataset(Dataset):
        ...
        def __getitem__(self, index):
            ...
            x, x_cat, x_scale = self.scaler.scale_input_1d(original_x)
            return (
                torch.from_numpy(x),
    ...
    def _make_dataset(self, mode="train"):
        ...
        def _process(source):
            for sample in source:
                ...
                x, x_cat, x_scale, y, mask = self.scaler.filter_and_scale(
                    original_x, original_y
                )
                x, x_cat, x_scale, y, mask, original_x, original_y = (
                    torch.tensor(x),
                    torch.tensor(x_cat),
                ...
                yield x, x_cat, x_scale, y, mask, original_x, original_y
...
class LeapModel(nn.Module):
    ...
    def forward(self, x, x_cat, x_scale):
...
class LeapLightningModule(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        mode = "valid"
        x, x_cat, x_scale, y, mask, original_x, original_y = batch
        x, x_cat, x_scale, y, mask, original_x, original_y = (
            torch.flatten(x, start_dim=0, end_dim=1),
            torch.flatten(x_cat, start_dim=0, end_dim=1),
            torch.flatten(x_scale, start_dim=0, end_dim=1),
    ...
    def predict_step(self, batch, batch_idx):
        mode = "test"
        x, x_cat, x_scale, _ = batch
        x = x.to(self.torch_dtype)
        x_scale = x_scale.to(self.torch_dtype)
        x_cat = x_cat.to(torch.long)
        out = self.__pred(x, x_cat, x_scale, mode)
        return out

    def __pred(self, x, x_cat, x_scale, mode: str) -> torch.Tensor:
        if (mode == "valid" or mode == "test") and (self.model_ema is not None):
            out = self.model_ema.module(x, x_cat, x_scale)
        else:
            out = self.model(
                x,
                x_cat,
    ...
    def on_validation_epoch_end(self):
        valid_preds = np.nan_to_num(
            np.concatenate(self.valid_preds, axis=0).astype(np.float64), nan=0.0
        )
...
def predict_valid(cfg: DictConfig, output_path: Path) -> None:
    ...
    model = model.to("cuda")
    model.eval()
    for x, x_cat, x_scale, _, _, original_x, original_y in tqdm(dataloader):
        x, x_cat, x_scale, original_x, original_y = (
            x.to("cuda"),
            x_cat.to("cuda"),
            x_scale.to("cuda"),
            original_x.to("cuda"),
...
def predict_test(cfg: DictConfig, output_path: Path) -> None:
    ...
    model = model.to("cuda")
    model.eval()
    for x, x_cat, x_scale, original_x in tqdm(dataloader):
        x = x.to("cuda")
        x_cat = x_cat.to("cuda")
