...
class GNN(nn.Module):
    ...
    def forward(self, x):
        ...
        result = result.view(
            -1, self.n_layers * self.base_channels, 60
        )  #  (384, n_layers * n_base_channels, 60)
        return result
...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.unet = UNet(
            n_channels=(1 + gnn_n_layers1) * same_height_hidden_sizes[-1],
        ...
        out_base_channels = (1 + gnn_n_layers2) * n_base_channels
        self.t_head = MLP(
            9 + out_base_channels,
