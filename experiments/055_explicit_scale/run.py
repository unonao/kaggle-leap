...
class Scaler:
    ...
    def get_explicit_scaler(self):
        # 360次元分のスケールを取得
        return self.feat_std_dict["base"][:360] / (1200.0 * self.y_rms_sub[:360])
...
class LeapModel(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.explicit_output_scaling is not None:
            x[:, :360] *= self.explicit_output_scaling
        return x
...
class LeapLightningModule(LightningModule):
    def __init__(self, cfg):
        ...
        if cfg.exp.explicit_output_scaling:
            explicit_output_scaling = (
                torch.from_numpy(self.scaler.get_explicit_scaler())
                .to(torch.float32)
                .to("cuda")
            )
