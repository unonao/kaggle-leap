...
class LeapModel(nn.Module):
    def __init__(
        ...
        self.t_head = MLP(
            8 + n_base_channels + radiation_hidden_sizes[-1],
            output_hidden_sizes + [1],
            use_layer_norm=use_output_layer_norm,
        )
        self.q1_head = MLP(
            4 + n_base_channels + radiation_hidden_sizes[-1],
    ...
    def forward(self, x, x_cat):
        ...
        x = torch.cat(
            [
                x_cloud[:, :60, :],
                x_radiation.unsqueeze(1).repeat(1, 60, 1),
            ],
            dim=2,
        )  # -> (batch, 60, n_base_channels + radiation_hidden_sizes[-1])
