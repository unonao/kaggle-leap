...
def make_sim_data_from_df(cfg, df, old_df, save_dir, mode):
    ...
    old_filter_df = (
        ...
        .filter((pl.col("time_mod") - 60) % 120 == 0)
        .drop(["index", "time_mod"])
    )
        ...
        stats_df = (
            ...
            .with_columns(
                [
                    pl.Series(
                        "old_row_index",
                        values=(((top_k_similar.numpy() // 384) * 4 * 30 + 60) * 384)
            ...
            .with_columns(
                [
                    ...
                    .list.slice(0, 1)
                    .list.contains(pl.col("row_index") + 384 * 12)
                    .alias("is_top1_next")
                ]
            )
            .with_columns(
                [
                    ...
                    for i in [12, -12, -36, -60, -84, -108, 36, 60, 84, 108]
                    for k in [1, 2, 3]
                ]
            )
        )
        all_size = len(stats_df)
        max_get_size = len(stats_df.filter(pl.col("time_mod") == 48))
    ...
    for start_ri in tqdm(range(0, len(base_array), 384)):
        ...
        if mode == "valid":
            ...
            is_in_bools[:, :4] = stats_df[start_ri:end_ri][
                [f"is_top1_{i}" for i in [12, -12, -36, 36]]
            ]
