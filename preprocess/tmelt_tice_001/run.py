...
def get_tmelt_tice(cfg, df: pl.DataFrame):
    ...
    for h in range(60):
        ...
        tmelt = df4[f"state_t_{h}"].max()
        tice = df4[f"state_t_{h}"].min()

        tmelt = (
            tmelt if (zero_q3 > 10) or (tmelt is None) else max(273.15, tmelt)
        )  # 温度が低くてそもそも zero_q3 が存在しないor数個しかないことがある. その場合は 273.15 以上にする
        tice = (
            tice if (zero_q2 > 10) or (tice is None) else min(193.15, tice)
        )  # 温度が低くてそもそも zero_q2 が存在しないor数個しかないことがある. その場合は 193.15 未満にする
        print(
            h,
