...
def process_features(x_array, y_array, pressures_array):
    grid_path = "/kaggle/working/misc/grid_info/ClimSim_low-res_grid-info.nc"
    grid_info = xr.open_dataset(grid_path)
    ...
    lhf_normalized = cal_normalized_lfh(
        x_array[:, 60:120],
        x_array[:, 0:60],
        x_array[:, 360],
        x_array[:, 362],
...
def cal_stats_x_y(df, features_dict, output_path):
    ...
    mean_feat_dict["buoyancy_all"] = np.nanmean(features_dict["buoyancy"])
    ...
    std_feat_dict["buoyancy_all"] = np.nanstd(features_dict["buoyancy"])
...
def main(cfg: DictConfig) -> None:
    ...
    features_dict = process_features(
        df[:, 1:557].to_numpy(), df[:, 557:].to_numpy(), pressures
    )
