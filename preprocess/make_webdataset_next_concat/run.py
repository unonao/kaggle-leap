...
def make_webdataset(cfg: DictConfig, exp_name) -> None:
    ...
    month_dirs = (
        ...
        [f"train/0004-{str(m).zfill(2)}" for m in range(2, 13)]
        + [f"train/000{y}-{str(m).zfill(2)}" for y in range(5, 9) for m in range(1, 13)]
        + ["train/0009-01"]
    )
    ...
    for month_dir in month_dirs:
        ...
        make_webdataset_month(cfg, exp_name, month_dir, data_utils)
